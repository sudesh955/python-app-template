...
class CmdArgsBuilder:
	_equal: ClassVar[str] = "="
	_scoper: ClassVar[str] = "."
	_scope_suffix: ClassVar[str] = ":"
	_array_placeholder: ClassVar[str] = "$"

	def __init__(self, txt: str) -> None:
		tokens = split_tokens(txt)
		self._cursor: int = 0
		self._current_scope: str = ""
		self._length: int = len(tokens)
		self._args: list[CmdToken] = []
		self._tokens: list[CmdToken] = tokens
...
def get_module_name(argv: list[str]) -> str:
	tail: str
	module: list[str] = []
	path: str = os.path.relpath(argv[1])[:-3]
	while path:
...
def get_params_type(
	method: str, fn: Callable[..., Any]
) -> tuple[bool, ParamsType]:
	s = signature(fn)
